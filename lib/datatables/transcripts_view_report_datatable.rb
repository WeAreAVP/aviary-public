...
class Datatables::TranscriptsViewReportDatatable < Datatables::ApplicationDatatable
  ...
  def fetch_indexes_view
    resource_views, count = AnalyticsReports.fetch_transcripts_view(page, per_page, sort_column, sort_direction, @current_organization.id, params)
    [resource_views, count]
  end

  def columns
    %w(file_transcripts.id file_transcripts.title MAX(ahoy_events.time) count(ahoy_events.id))
  end
end
