...
function UserRegister() {
    ...
    const check_password_match = function (pswd) {
        ...
        if (pswd == $(parent_container + ' #user_password').val() && pswd != '') {
            ...
            $(parent_container + ' .validation_password_match i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        } else {
            flag = false;
            $(parent_container + ' .validation_password_match').removeClass('valid-signup').addClass('invalid-signup');
            $(parent_container + ' .validation_password_match i').removeClass('fa-check').addClass('fa-times').attr("aria-label","invalid");
        }
        return flag;
    };
    ...
    const check_password_strength = function (pswd) {
        ...
        if (pswd.length < 8) {
            ...
            $(parent_container + ' .validation_length i').removeClass('fa-check').addClass('fa-times').attr("aria-label","invalid");
            flag = false;
        } else {
            $(parent_container + ' .validation_length').removeClass('invalid-signup').addClass('valid-signup');
            $(parent_container + ' .validation_length i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        }

        //validate letter
        if (pswd.match(/[A-z]/)) {
            $(parent_container + ' .validation_letter').removeClass('invalid-signup').addClass('valid-signup');
            $(parent_container + ' .validation_letter i').removeClass('fa-check').addClass('fa-times').attr("aria-label","invalid");
        } else {
            flag = false;
            $(parent_container + ' .validation_letter').removeClass('valid-signup').addClass('invalid-signup');
            $(parent_container + ' .validation_letter i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        }

        //validate uppercase letter
        if (pswd.match(/[A-Z]/)) {
            $(parent_container + ' .validation_capital').removeClass('invalid-signup').addClass('valid-signup');
            $(parent_container + ' .validation_capital i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        } else {
            flag = false;
            $(parent_container + ' .validation_capital').removeClass('valid-signup').addClass('invalid-signup');
            $(parent_container + ' .validation_capital i').removeClass('fa-check').addClass('fa-times').attr("aria-label","invalid");
        }
        //validate uppercase letter
        if (pswd.match(/[a-z]/)) {
            $(parent_container + ' .validation_lower').removeClass('invalid-signup').addClass('valid-signup');
            $(parent_container + ' .validation_lower i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        } else {
            flag = false;
            $(parent_container + ' .validation_lower').removeClass('valid-signup').addClass('invalid-signup');
            $(parent_container + ' .validation_lower i').removeClass('fa-check').addClass('fa-times').attr("aria-label","invalid");
        }
        //special characters
        if (pswd.match(/[.@$!%*?&]/)) {
            $(parent_container + ' .validation_special_character').removeClass('invalid-signup').addClass('valid-signup');
            $(parent_container + ' .validation_special_character i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        } else {
            flag = false;
        ...
        if (pswd.match(/[#()_+=<>/"'{}\[\]\\|`~;:\^\-]/)) {
            ...
            $(parent_container + ' .validation_can_only_have_special_character i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        }

        //validate number
        if (pswd.match(/\d/)) {
            $(parent_container + ' .validation_number').removeClass('invalid-signup').addClass('valid-signup');
            $(parent_container + ' .validation_number i').removeClass('fa-times').addClass('fa-check').attr("aria-label","valid");
        } else {
            flag = false;
            $(parent_container + ' .validation_number').removeClass('valid-signup').addClass('invalid-signup');
            $(parent_container + ' .validation_number i').removeClass('fa-check').addClass('fa-times').attr("aria-label","invalid");
        }
        return flag;
    };
