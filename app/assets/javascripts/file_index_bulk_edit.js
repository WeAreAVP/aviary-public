...
function FileIndexBulkEdit(ids_session_raw) {
    ...
    const get_progress_status = function () {
        selfRBE.app_helper.classAction($('#url_form_progress').data('url'), {
            action: 'update_progress_indexes',
    ...
    const binging_select_all_records = function () {
        ...
        document_level_binding_element('#select_all_records', 'click',function () {
            ...
            const searchval = $('#indexes_datatable_filter label input').val()
            var data = {
    ...
    const updateCount = function (number_selected) {
        ...
        if (number_selected > 0) {
            ...
            $('#indexes_datatable_filter label').append('<span style="color:#204f92" class="ml-10px font-weight-bold" id="select_all_records_span">| <a href="javascript://" class="font-weight-bold" id="select_all_records" data-url="/collections/bulk_resource_list">Select All (' + selfRBE.total_records +' records)</a> </span>');
