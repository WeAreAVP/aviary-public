...
function TranscriptTable() {
    ...
    this.initialize = function (_table_column_detail, _list_bulk_edit) {
        ...
        that._bulk_edit = new TranscriptBulkEdit(that._list_bulk_edit);
    ...
    this.outSourceDataTable = function (called_from, caller, additionalData, tableName = 'transcripts_datatable') {
    ...
    const initDataTable = function (called_from, caller, additionalData, tableName) {
        ...
        if (!tableName && typeof tableName == 'undefined') {
            tableName = 'transcripts_datatable';
        }
        ...
        if (dataTableElement.length > 0) {
            that.dataTableObj = dataTableElement.DataTable({
                ...
                language: {
                    ...
                    infoFiltered: '',
                    zeroRecords: 'No Transcript found.',
                    lengthMenu: " _MENU_ "
                },
                order: [[1, 'asc']],
                columnDefs: [
                ...
                drawCallback: function (settings) {
                    if (called_from == 'permission_group') {
                        ...
                        $('.add_to_transcript_group').unbind('click');
                        $('.add_to_transcript_group').on('click', function () {
                            let permission_group = new PermissionGroup();
                            let response = permission_group.outsourceAddToken('permission_group_transcripts', $(this).data());
                            if (response) {
                                jsMessages('success', 'Transcript added to permission group');
                            }
                        });
                    } else {
                        initDeletePopup();
    ...
    const initDeletePopup = function () {
        $('.transcript_delete').click(function () {
            $('#modalPopupTitle').html('Delete "' + $(this).data().name + '" Transcripts');
            $('#modalPopupBody').html('Are you sure you want to delete this transcript? There is no undoing this action.');
