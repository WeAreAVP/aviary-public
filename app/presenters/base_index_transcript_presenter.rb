...
class BaseIndexTranscriptPresenter < BasePresenter
  def add_marker_occurrences(session_video_text_all, text)
    ...
    session_video_text_all.each do |key_keyword, single_keyword|
      occurrences = text.scan(/#{single_keyword}/i).uniq
      occurrences_others = text.scan(/\b#{single_keyword}\b/i).uniq
      occurrences += occurrences_others if occurrences_others.present?
      occurrences = occurrences.flatten
