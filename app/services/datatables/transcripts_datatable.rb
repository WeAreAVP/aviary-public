...
class TranscriptsDatatable < ApplicationDatatable
  delegate :time_to_duration, :number_to_human_size, :strip_tags, :check_valid_array, :options_for_select, :select_tag, :content_tag, :noid_url, :embeded_url, :collection_resource_url, :embed_file_url,
           ...
           :user_remove_user_path, :collection_collection_resource_details_path, :collection_collection_resource_details_url, :transcript_delete_path, :can?, to: :@view
      ...
      [].tap do |column|
        ...
        links = if %w[permission_group playlist_add_resource].include?(@called_from)
                  ...
                  begin
                    ...
                    link_set += "<a href='javascript://' data-name='#{strip_tags(resource['title_ss'].to_s)}' data-url='#{transcript_delete_path(resource['id_is'])}' class='btn-sm btn-danger transcript_delete' >Delete</a>"
                    link_set
                  rescue StandardError => e
                    puts e.backtrace.join("\n")
                    'Nouman'
