...
RSpec.describe Aviary::PreviewScript, type: :service do
  ...
  describe '#data_hash' do
    ...
    context 'when resource is present' do
      subject { Aviary::PreviewScript.new(collection).data_hash(collection_resource, nil) }
    ...
    context 'when resource is not present' do
      subject { Aviary::PreviewScript.new(collection).data_hash(nil,  nil) }
  ...
  describe '#update_data_hash' do
    context 'when resource is present' do
      ...
      it 'it return resource name in hash' do
        test = Aviary::PreviewScript.new(collection, params).update_data_hash(Aviary::PreviewScript.new(collection).data_hash(collection_resource, nil))
  ...
  describe '#preview_hash_switch' do
    ...
    context 'when system value is title' do
      ...
      subject { Aviary::PreviewScript.new(collection, params).preview_hash_switch('title') }
    ...
    context 'when system value is rights_statement' do
      ...
      subject { Aviary::PreviewScript.new(collection, params).preview_hash_switch('rights_statement') }
    ...
    context 'when system value is publisher' do
      ...
      subject { Aviary::PreviewScript.new(collection, params).preview_hash_switch('publisher') }
