class UpdateResourceFileDisplayColumnValue < ActiveRecord::Migration[6.1]
  def change
    Organization.all.each do |org|
      ...
      if resource_file_display_column.present?
        ...
        begin
          unless display_keys.include?('embed_code_texts')
            resource_file_display_column['columns_status'][resource_file_display_column['columns_status'].count] = { 'status' => 'true', 'value' => 'embed_code_texts', 'sort_name' => 'true' }
          end
    ...
    failed = []
    CollectionResourceFile.order(id: :desc).all.each do |obj|
      begin
        Sunspot.index obj
        Sunspot.commit
        puts obj.id
