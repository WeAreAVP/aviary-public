class AddDownloadableToTranscript < ActiveRecord::Migration[6.1]
  def change
    add_column :file_transcripts, :is_downloadable, :integer, default: 1
      ...
      if transcript_display_column.present? && transcript_search_column.present?
        ...
        begin
          unless display_keys.include?('is_downloadable_ss')
            transcript_display_column['columns_status'][transcript_display_column['columns_status'].count] = { 'status' => 'true', 'value' => 'is_downloadable_ss', 'sort_name' => 'true' }
          end
          unless display_keys.include?('is_downloadable_ss')
            transcript_search_column[transcript_search_column.count] = { 'status' => 'true', 'value' => 'is_downloadable_ss' }
          end
