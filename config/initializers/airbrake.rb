...
Airbrake.configure do |c|
  ...
  c.blacklist_keys = [/password/i, /password_confirmation/i, /authorization/i]
...
Airbrake.add_filter do |notice|
  if notice[:errors].present? && notice[:errors].try(:first).present?
    error = notice[:errors].first
    notice.ignore! if error[:type] == 'RecordNotFound'
    notice.ignore! if error[:type] == 'ActiveRecord::RecordNotFound'
    notice.ignore! if error[:type] == 'ActionController::UnknownFormat'
    #notice.ignore! if error[:type] == 'ActionController::InvalidAuthenticityToken'
  end
end
